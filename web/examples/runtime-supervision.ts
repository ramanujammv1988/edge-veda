/**
 * Edge Veda Web SDK — Runtime Supervision (Phase 4) Example
 *
 ...
 * thermal monitoring, battery drain tracking, and telemetry — all running
 * in the browser.
 *
 * Usage:
 * ```typescript
 * import { runRuntimeSupervisionExample } from './runtime-supervision';
 * runRuntimeSupervisionExample();
 * ```
 *
 * Or from an HTML page:
 * ```html
 * <script type="module">
 *   import { runRuntimeSupervisionExample } from './runtime-supervision.js';
 *   runRuntimeSupervisionExample();
 * </script>
 * ```
 */
...
export async function runRuntimeSupervisionExample(): Promise<void> {
  console.log('EdgeVeda Web SDK — Runtime Supervision (Phase 4)');
  console.log('=================================================\n');
  ...
  // 6. Resource Monitor
  // ─────────────────────────────────────────────────────
  console.log('6. Resource Monitor (JS Heap)');
  console.log('─────────────────────────────');
  ...
  console.log(`  Supported:    ${resourceMonitor.isSupported}`);
  console.log(`  Current heap: ${currentRss.toFixed(1)} MB`);
  console.log(`  Peak heap:    ${peakRss.toFixed(1)} MB`);
  console.log(`  Average heap: ${avgRss.toFixed(1)} MB`);
  console.log(`  Heap limit:   ${resourceMonitor.heapLimitMb.toFixed(1)} MB`);
  ...
  // Manual thermal level update (browser doesn't have native thermal API)
  ...
  // In browser, try the Battery Status API
  try {
    await batteryTracker.sampleFromAPI();
  } catch {
    // Battery API not available in all browsers
    console.log('  (Battery Status API not available — using manual samples)');
  }

  // Manual sample fallback
  ...
  console.log('  ✅ ResourceMonitor (JS heap, peak, average)');
  console.log('  ✅ ThermalMonitor (level, manual update, listeners)');
  console.log('  ✅ BatteryDrainTracker (Battery Status API + manual samples)');
