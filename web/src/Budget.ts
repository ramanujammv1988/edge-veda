/**
 * Edge Veda Web SDK - Budget Types
 * Declarative compute budget contracts for browser-based inference.
 *
 ...
 * Note: Battery and thermal monitoring have limited browser support.
 */
export interface EdgeVedaBudget {
  ...
  /** Maximum memory heap usage in megabytes. Undefined to skip memory enforcement. */
 ...
 *
 * The Scheduler builds this after its LatencyTracker has collected sufficient data.
 */
export interface MeasuredBaseline {
  ...
  /** Current JS heap usage in megabytes at time of measurement. */
  currentRssMb: number;
...
export enum BudgetConstraint {
  ...
  /** Memory usage exceeded the declared ceiling. */
...
export enum WorkloadId {
  ...
  /** Text/chat inference (via ChatSession). */
  TEXT = 'text',
}
...
export class Budget {
  ...
  static validate(budget: EdgeVedaBudget): string[] {
    ...
    if (
      ...
      warnings.push(
        `p95LatencyMs=${budget.p95LatencyMs} is likely unrealistic for browser-based LLM inference ` +
          '(typical: 1000-5000ms depending on model and device)'
      );
    }
    ...
    if (
      ...
      budget.memoryCeilingMb < 500
    ) {
      warnings.push(
        `memoryCeilingMb=${budget.memoryCeilingMb} may be too low for browser-based LLM inference ` +
          '(typical JS heap: 500-2000MB including model weights + WebGPU buffers). ' +
          'Consider setting to undefined to skip memory enforcement.'
      );
    }
  ...
  static baselineToString(baseline: MeasuredBaseline): string {
    ...
    return (
      ...
      `heap=${baseline.currentRssMb.toFixed(0)}MB, samples=${baseline.sampleCount})`
    );
  }
