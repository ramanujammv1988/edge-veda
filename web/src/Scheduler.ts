 ...
 * Async/await provides the same sequential guarantees as Swift's actors
 * and Kotlin's Mutex.
 *
 * @example
 * ```typescript
...
export class Scheduler {
  ...
  async scheduleTask<T>(
    ...
    try {
      ...
      // Record latency (using performance.now() for high-resolution timing)
      ...
      console.log(
        `[EdgeVeda.Scheduler] Task completed: ${taskId} duration=${durationMs.toFixed(1)}ms`
      );
      return result;
    } catch (error) {
      console.error(
