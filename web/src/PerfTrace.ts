/**
 * PerfTrace — JSONL frame-based trace logger for Web
 *
 * Records performance trace events with frame IDs, timestamps, stages, and values.
 * Stores records in-memory and supports JSONL string export and Blob download.
 *
...
export class PerfTrace {
  ...
  /**
   * Export trace as a downloadable Blob (MIME type application/x-ndjson).
   * Useful for saving trace files from the browser.
   */
  exportBlob(): Blob {
    const jsonl = this.exportJSONL();
    return new Blob([jsonl], { type: 'application/x-ndjson' });
  }

  /**
   * Trigger a browser download of the trace JSONL file.
   *
   * @param filename  Name for the downloaded file.
   *                  Defaults to `trace-<timestamp>.jsonl`.
   */
  download(filename?: string): void {
    const blob = this.exportBlob();
    const url = URL.createObjectURL(blob);
    const name = filename ?? `trace-${Date.now()}.jsonl`;
