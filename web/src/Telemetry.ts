 ...
 *
 * JavaScript is single-threaded in the browser, so no mutex is needed.
 * Uses `performance.now()` for high-resolution timestamps where appropriate.
 *
 * @example
 * ```typescript
...
export interface LatencyMetric {
  requestId: string;
  modelName?: string;
  startTime?: number; // performance.now() timestamp for high-resolution timing
  endTime?: number;
  latencyMs?: number;
}
...
export interface BudgetViolationRecord {
  ...
  wallTime: Date; // wall-clock time for display
 ...
 * - Uses `performance.now()` for high-resolution monotonic timestamps
 * - Stores both monotonic and wall-clock times for correlation
...
export class Telemetry {
  ...
  logInferenceComplete(requestId: string, tokensGenerated?: number): void {
    ...
    if (metric) {
      ...
      const latencyMs =
        metric.startTime !== undefined ? endTime - metric.startTime : undefined;
      this.latencyMetrics.set(requestId, {
        ...metric,
        endTime,
        latencyMs,
  ...
  logInferenceError(requestId: string, error: string): void {
    console.error(
      `[${TAG_INFERENCE}] Inference failed: ${requestId} error=${error}`
    );
  }
  ...
  recordLatency(requestId: string, latencyMs: number): void {
    console.debug(
      `[${TAG_INFERENCE}] Latency recorded: ${requestId} latency=${latencyMs}ms`
    );
  ...
  logBudgetEnforcement(action: string, reason: string): void {
    console.info(
      `[${TAG_BUDGET}] Budget enforcement: ${action} reason=${reason}`
    );
  }
  ...
  logThermalStateChange(from: number, to: number): void {
    console.info(
      `[${TAG_RESOURCES}] Thermal state changed: ${from} -> ${to}`
    );
  }
  ...
  logTaskScheduled(taskId: string, priority: string): void {
    console.debug(
      `[${TAG_SCHEDULER}] Task scheduled: ${taskId} priority=${priority}`
    );
  }
  ...
  logTaskCompleted(taskId: string, durationMs: number): void {
    console.debug(
      `[${TAG_SCHEDULER}] Task completed: ${taskId} duration=${durationMs}ms`
    );
  }
  ...
  logTaskCancelled(taskId: string, reason: string): void {
    console.info(
      `[${TAG_SCHEDULER}] Task cancelled: ${taskId} reason=${reason}`
    );
  }
  ...
  logPolicyEnforcement(action: string, context: string): void {
    console.info(
      `[${TAG_POLICY}] Policy enforcement: ${action} context=${context}`
    );
  }
  ...
  clearOldMetrics(olderThanMs: number): void {
    const cutoffPerf = performance.now() - olderThanMs;

    for (const [key, metric] of this.latencyMetrics) {
      if (metric.startTime === undefined || metric.startTime < cutoffPerf) {
        this.latencyMetrics.delete(key);
      }
    }

    this.budgetViolations = this.budgetViolations.filter(
      (v) => v.timestamp >= cutoffPerf
    );
    this.resourceSnapshots = this.resourceSnapshots.filter(
      (s) => s.timestamp >= cutoffPerf
    );

    console.info(
      `[${TAG_INFERENCE}] Cleared metrics older than ${olderThanMs}ms`
    );
  }
  ...
  private trimViolations(): void {
    if (this.budgetViolations.length > this.maxStoredViolations) {
      const toRemove =
        this.budgetViolations.length - this.maxStoredViolations;
  ...
  private trimSnapshots(): void {
    if (this.resourceSnapshots.length > this.maxStoredSnapshots) {
      const toRemove =
        this.resourceSnapshots.length - this.maxStoredSnapshots;
