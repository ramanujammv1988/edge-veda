...
import { CancelToken, GenerateOptions, GenerationError } from './types';
...
export class ChatSession {
  ...
  async send(
    ...
    try {
      ...
      const result = await this.edgeVeda.generate({ ...options, prompt });
      ...
      if (this.messages.length > 0 && this.messages[this.messages.length - 1].role === ChatRole.USER) {
        this.messages.pop();
      }
      throw e;
    }
  ...
  async *sendStream(
    ...
    try {
      ...
      for await (const chunk of this.edgeVeda.generateStream({ ...options, prompt })) {
        cancelToken?.throwIfCancelled();
   ...
   *
   * Tool call and result messages are added to `allMessages` for inspection.
   *
  ...
  async sendWithTools(
    ...
    try {
      ...
      for (let round = 0; round < maxToolRounds; round++) {
        ...
        const toolCalls = ToolTemplate.parseToolCalls(this._toolFormat, result.text);
        ...
        // Process the first tool call (single-call per round, like Flutter SDK)
        ...
        // Loop continues — model will see the tool result next round
      }

      // Max rounds exhausted — generate a final response without tool parsing
      cancelToken?.throwIfCancelled();
      const finalPrompt = this._formatConversationWithTools();
      const finalResult = await this.edgeVeda.generate({ ...options, prompt: finalPrompt });
  ...
  async sendStructured(
    ...
    const grammarOptions: GenerateOptions = {
      ...options,
      prompt: '', // will be overridden by send()
      grammarStr,
      grammarRoot: 'root',
    };
  ...
  private _formatConversationWithTools(): string {
    ...
    // Replace the system message with the tool-augmented version
  ...
  private async _summarizeIfNeeded(cancelToken?: CancelToken): Promise<void> {
    ...
    try {
      ...
      const summaryResult = await this.edgeVeda.generate({
      ...
      const targetTokens = Math.floor((this.maxContextLength - this.maxResponseTokens) * 0.6);
  ...
  /**
   * Estimates the total token count for all messages (~4 chars per token).
   */
  ...
  /**
   * Derive the ToolTemplateFormat that matches the current ChatTemplate.
   * Qwen3 → QWEN3, Gemma3 → GEMMA3, everything else → GENERIC.
   */
