...
// Mock navigator.getBattery to prevent constructor side-effects
...
describe('BatteryDrainTracker', () => {
  ...
  // ---------------------------------------------------------------------------
  // sampleFromAPI - no battery manager
  // ---------------------------------------------------------------------------

  test('sampleFromAPI is a no-op when Battery API unavailable', async () => {
    await tracker.sampleFromAPI();
    // Constructor couldn't get battery manager, so no sample added beyond initial
    expect(tracker.sampleCount).toBe(0);
  });
});