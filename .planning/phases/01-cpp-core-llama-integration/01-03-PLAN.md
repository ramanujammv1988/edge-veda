...
# Verify llama.cpp symbols are present in the binary
# This confirms the library actually contains llama.cpp code, not just stubs
# CRITICAL: This is a hard requirement - build fails if symbols are missing
echo ""
...
# Verify Metal framework linking (device only)
echo ""
echo "=== Metal framework check (device library) ==="
DEVICE_LIB=$(find flutter/ios/Frameworks/EdgeVedaCore.xcframework -path "*ios-arm64/*" -name "*.a" | head -1)
if [ -n "$DEVICE_LIB" ]; then
    nm "$DEVICE_LIB" 2>/dev/null | grep -c "MTL\|Metal" && echo "Metal symbols present" || echo "No Metal symbols (may be OK if dynamically linked)"
fi
```
  </verify>
  <done>
...
- Info.plist present
- llama.cpp symbols verified in binary (>= 10 symbols for llama_init, llama_load_model, llama_decode, llama_free)
- Verification exits with error if symbols < 10 (hard requirement)
  </done>
</task>
...
5. **Symbols verified (HARD REQUIREMENT):**
   ```bash
   for lib in $(find flutter/ios/Frameworks/EdgeVedaCore.xcframework -name "*.a"); do
       LLAMA_SYMBOLS=$(nm "$lib" 2>/dev/null | grep -c "llama_")
       if [ "$LLAMA_SYMBOLS" -lt 10 ]; then
           echo "FAIL: $lib has only $LLAMA_SYMBOLS llama symbols (need >= 10)"
           exit 1
       fi
       echo "PASS: $lib has $LLAMA_SYMBOLS llama symbols"
   done
   ```
</verification>
...
5. llama.cpp symbols present in binary (>= 10 symbols verified with nm, exits with error if not met)
</success_criteria>
