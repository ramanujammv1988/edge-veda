...
<task type="auto">
  ...
  <action>
Update model_manager.dart to implement crash-safe download with caching:

**CRITICAL: Add cache check FIRST (R2.3 - skip re-download if valid model exists):**
```dart
Future<String> downloadModel(ModelInfo model, {CancelToken? cancelToken}) async {
  ...
  // CHECK CACHE FIRST - skip download if valid model exists
  ...
  // Proceed with download only if not cached or invalid
