...
Tap the **assessment icon** (📊) to run a 10-test benchmark:
...
2. Read BENCHMARK.md and replace ALL `[VALUE from BENCHMARK.md]` and `[VALUE]` placeholders with actual numeric values from benchmark results (e.g., "18.5 tok/s", "320 ms", "950 MB")
...
8. No placeholder brackets remain: `grep "\[VALUE" flutter/example/README.md` returns empty (all placeholders replaced with actual numbers)
  </verify>
  <done>
