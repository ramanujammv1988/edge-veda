...
class EdgeVedaModule(reactContext: ReactApplicationContext) : 
    ...
    @ReactMethod
    fun initVision(config: String, promise: Promise) {
        moduleScope.launch {
            try {
                val configJson = JSONObject(config)
    ...
    @ReactMethod
    fun describeImage(
        rgbBytes: String, width: Int, height: Int,
    ...
    @ReactMethod
    fun freeVision(promise: Promise) {
        moduleScope.launch {
            visionContext?.free()
            visionContext = null
            promise.resolve(null)
    ...
    @ReactMethod(isBlockingSynchronousMethod = true)
    fun isVisionLoaded(): Boolean = visionContext != null
    ...
    @ReactMethod
    fun initWhisper(modelPath: String, config: String, promise: Promise) {
        moduleScope.launch {
            try {
                val configJson = JSONObject(config)
    ...
    @ReactMethod
    fun transcribeAudio(pcmBase64: String, nSamples: Int, params: String, promise: Promise) {
        moduleScope.launch {
            try {
    ...
    @ReactMethod
    fun freeWhisper(promise: Promise) {
        moduleScope.launch {
            whisperContext?.free()
            whisperContext = null
            promise.resolve(null)
    ...
    @ReactMethod(isBlockingSynchronousMethod = true)
    fun isWhisperLoaded(): Boolean = whisperContext != null
    ...
    @ReactMethod
    fun initImageGeneration(modelPath: String, config: String, promise: Promise) {
        moduleScope.launch {
            try {
                val configJson = JSONObject(config)
    ...
    @ReactMethod
    fun generateImage(params: String, promise: Promise) {
        moduleScope.launch {
            try {
                val engine = imageContext ?: throw IllegalStateException("Image generation context not initialized")
                val paramsJson = JSONObject(params)

                // Progress callback → emit EdgeVeda_ImageProgress events
    ...
    @ReactMethod
    fun freeImageGeneration(promise: Promise) {
        moduleScope.launch {
            imageContext?.free()
            imageContext = null
            promise.resolve(null)
    ...
    @ReactMethod(isBlockingSynchronousMethod = true)
    fun isImageGenerationLoaded(): Boolean = imageContext != null
